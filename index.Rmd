...
Each plot has labels. The `title` should containg a short sentence summarizing what the plot is showing. The `subtitle` should contain what is actually shown (e.g., plottype, variables, etc.). The `caption` should give additional information that might make the plot ambigious to read. Additionally sources can be added here.

The bins of a histogram become more readable, when the border color is set to `"white"` as in this exapmle, as it delineates the plot from background.
```{r histogram-1}
df %>%
  select(age) %>%
  ggplot() +
